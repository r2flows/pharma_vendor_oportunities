    ...
    if not pos_list:
        ...
        # 2. Filtrar productos ganadores locales que corresponden al POS seleccionado
            ...
            if not intersection.empty:
                ...
                elif 'unidades_pedidas' in intersection.columns and 'precio_minimo' in intersection.columns:
                    intersection['precio_total_droguería'] = intersection['unidades_pedidas'] * intersection['precio_minimo']
                    orders_total = intersection['precio_total_droguería'].sum()
...
# Preparar datos para vendor analysis
    ...
    # Crear gráfico
        ...
        if not summary_df.empty:
                st.subheader('Insights Accionables por Status de Vendor')
                
                # Pestañas para cada tipo de status
                tab1, tab2, tab3 = st.tabs(["Sin Relación Comercial", "Activos", "Pendientes"])

                # Mostrar datos según el status y el valor potencial
                with tab1:
                    filtered_df = summary_df[(summary_df['status'].isna())&(summary_df['valor_potencial'] > 20000)]
                    if not filtered_df.empty:
                        st.dataframe(filtered_df[[
                            'point_of_sale_id', 'vendor_id', 'valor_potencial', 'tipo_oportunidad', 'porcentaje_ahorro'
                        ]].style.format({
                            'valor_potencial': '${:,.2f}',
                            'porcentaje_ahorro': '{:.2f}%'
                        }))
                    else:
                        st.write("No hay datos disponibles para esta categoría.")
                        
                with tab2:
                    filtered_df = summary_df[(summary_df['status']==1)&(summary_df['valor_potencial'] > 20000)]
                    if not filtered_df.empty:
                        st.dataframe(filtered_df[[
                            'point_of_sale_id', 'vendor_id', 'valor_potencial', 'tipo_oportunidad', 'porcentaje_ahorro'
                        ]].style.format({
                ...
                with tab3:
                    filtered_df = summary_df[(summary_df['status']==2)&(summary_df['valor_potencial'] > 20000)]
                    if not filtered_df.empty:
                        st.dataframe(filtered_df[[
                            'point_of_sale_id', 'vendor_id', 'valor_potencial', 'tipo_oportunidad', 'porcentaje_ahorro'
                        ]].style.format({
